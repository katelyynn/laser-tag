...
execute unless score can_start_players internal matches 1.. run tellraw @a ["",{"text":"[","color":"red"},{"text":"X","color":"red","bold":true},{"text":"] ","color":"red"},{"text":"Cannot start, at least 2 players required.","color":"red"}]
# period check
execute if score period internal matches -1 run scoreboard players set can_start_period internal 1
execute unless score can_start_period internal matches 1.. run tellraw @a ["",{"text":"[","color":"red"},{"text":"X","color":"red","bold":true},{"text":"] ","color":"red"},{"text":"Cannot start, a game is already in progress.","color":"red"}]
